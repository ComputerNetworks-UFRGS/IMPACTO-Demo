...
for i in range(0,6):
    ...
    axs[i//3, i%3].set_title('Potência = {:.2f}'.format(i))
...
for i in range(0, 6):
    ...
    max_peso = max(pesos[i])
    ax.scatter(valores, [i]*len(valores), tentativas, alpha=np.array(pesos[i])/max_peso, color='blue')

    Vs.set_media_try(A_tentativas[i] * Vs.get_log_value() + B_tentativas[i])
    ax.scatter(Vs.get_log_value(), i, Vs.percent_try(), color='black')
    ax.scatter(Vs.get_log_value(), i, Vs.percent_try() + A_tentativas_desvio[i] * Vs.get_log_value() + B_tentativas_desvio[i], color='black')
    ax.scatter(Vs.get_log_value(), i, Vs.percent_try() - A_tentativas_desvio[i] * Vs.get_log_value() - B_tentativas_desvio[i], color='black')

    ax.plot(x_tentativas, [i]*len(x_tentativas), y_tentativas, color='blue')
    ax.plot(x_tentativas, [i]*len(x_tentativas), y_tentativas_up, '--', color='royalblue')
    ax.plot(x_tentativas, [i]*len(x_tentativas), y_tentativas_down, '--', color='royalblue')
...
for i in range(0, 6):
    ...
    max_peso = max(pesos[i])
    ax.scatter(valores, [i]*len(valores), ataques, alpha=np.array(pesos[i])/max_peso, color='red')

    Vs.set_media_try(A_ataques[i] * Vs.get_log_value() + B_ataques[i])
    ax.scatter(Vs.get_log_value(), i, Vs.percent_try(), color='black')
    ax.scatter(Vs.get_log_value(), i, Vs.percent_try() + A_ataques_desvio[i] * Vs.get_log_value() + B_ataques_desvio[i], color='black')
    ax.scatter(Vs.get_log_value(), i, Vs.percent_try() - A_ataques_desvio[i] * Vs.get_log_value() - B_ataques_desvio[i], color='black')

    ax.plot(x_ataques, [i]*len(x_ataques), y_ataques, color='red')
    ax.plot(x_ataques, [i]*len(x_ataques), y_ataques_up, '--', color='lightcoral')
    ax.plot(x_ataques, [i]*len(x_ataques), y_ataques_down, '--', color='lightcoral')
...
for i in range(0, 6):
    ...
    max_peso = max(pesos[i])
    ax.scatter(valores, [i]*len(valores), danos, alpha=np.array(pesos[i])/max_peso, color='green')

    Vs.set_media_try(A_danos[i] * Vs.get_log_value() + B_danos[i])
    ax.scatter(Vs.get_log_value(), i, Vs.percent_try(), color='black')
    ax.scatter(Vs.get_log_value(), i, Vs.percent_try() + A_danos_desvio[i] * Vs.get_log_value() + B_danos_desvio[i], color='black')
    ax.scatter(Vs.get_log_value(), i, Vs.percent_try() - A_danos_desvio[i] * Vs.get_log_value() - B_danos_desvio[i], color='black')

    ax.plot(x_danos, [i]*len(x_danos), y_danos, color='green')
    ax.plot(x_danos, [i]*len(x_danos), y_danos_up, '--', color='lightgreen')
    ax.plot(x_danos, [i]*len(x_danos), y_danos_down, '--', color='lightgreen')
